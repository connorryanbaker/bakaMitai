...
func evalPieceSquares(b board.Board) float64 {
	...
	e += whiteKnightSqValues(b) * 3
	e += blackKnightSqValues(b) * -3
	e += whiteBishopSqValues(b) * 3
	e += blackBishopSqValues(b) * -3
	e += whiteRookSqValues(b) * 5
	e += blackRookSqValues(b) * -5
	e += whiteKingSqValues(b) * 10
	e += blackKingSqValues(b) * -10
